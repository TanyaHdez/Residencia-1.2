...
app.post('/login', loginLimiter, async (req, res) => {
  ...
  pool.query(query, [correo], async (err, results) => {
    ...
    req.session.user = {
      id: usuario.id,      // ID del usuario (Paciente o doctor)
      codigo: usuario.codigo,  // Código del usuario (CC2024 o CC2025)
      nombre: usuario.nombre,  // Nombre u otros datos que necesites
    };
    console.log('✅ Usuario guardado en sesión:', req.session.user); // 🛠 LOG

              if (usuario.codigo === 'CC2025') {
                // Si el usuario es paciente con código 'CC2025', verifica si tiene datos en historial_clinico
                ...
                pool.query(historialQuery, [usuario.id], (err, historialResults) => {
                  ...
                  if (historialResults.length > 0) {
                    // Si el paciente tiene datos en historial_clinico, redirigir a mi-perfil.html
                    const perfilId = historialResults[0].id;  //  id del paciente
                    console.log("➡️ Redirigiendo a:", `/mi-perfil/${perfilId}`);
                    res.json({ success: true, redirectUrl: `/mi-perfil/${perfilId}` });
                  } else {
                    // Si no tiene datos en historial_clinico, redirigir a página de inicio
                ...
                // Si el usuario es doctor con código 'CC2024', verifica si tiene datos en pacientes
                ...
                pool.query(perfilQuery, [usuario.id], (err, perfilResults) => {
                  ...
                  if (perfilResults.length > 0) {
                    // Si el doctor tiene datos en pacientes, redirigir a su perfil
                    console.log("➡️ Redirigiendo a la página de perfil del doctor");
                    res.json({ success: true, redirectUrl: '/inicio.html' });
                  } else {
                    // Si no tiene datos en pacientes, redirigir a la página de inicio del doctor
